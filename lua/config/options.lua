...
--编码--
vim.opt.encoding = "utf-8"                           --vim内部使用的字符编码
...
vim.opt.inccommand = "nosplit"     --增量搜索时不拆分窗口，在当前窗口中直接显示匹配项
...
--弹出式菜单--
vim.opt.pumheight = 10 --高度，即菜单项的最大树木
vim.opt.pumblend = 10  --透明度为10%
...
-- vim.opt.showcmd --在底部状态栏显示正在执行的命令，如选中几行后，显示选中的行数
